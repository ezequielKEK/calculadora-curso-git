  ...
  test('Memoria se inicializa en 0', () => {
    ...
    expect(nuevaCalc.getMemoria()).toBe(0);
  });
  test('Memoria guarda el resultado de una suma', () => {
    const nuevaCalc = new Calculadora();
    nuevaCalc.setMemoria(nuevaCalc.sumar(10, 5)); 
    expect(nuevaCalc.getMemoria()).toBe(15);
  });
  test('Memoria se sobrescribe con el resultado de una resta', () => {
    const nuevaCalc = new Calculadora();
    nuevaCalc.setMemoria(100); // Establece un valor inicial
    nuevaCalc.setMemoria(nuevaCalc.restar(10, 5));
    expect(nuevaCalc.getMemoria()).toBe(5);
  });

  test('Limpiar memoria la devuelve a cero', () => {
    const nuevaCalc = new Calculadora();
    nuevaCalc.setMemoria(123.45);
    nuevaCalc.limpiarMemoria(); // Usando el método que definiste en Calculadora.js
    expect(nuevaCalc.getMemoria()).toBe(0);
  });
  // Resumen
  console.log('\n=== Resumen ===');
